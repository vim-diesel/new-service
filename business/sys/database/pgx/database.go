...
func WithinTran(ctx context.Context, log *slog.Logger, db *sqlx.DB, fn func(*sqlx.Tx) error) error {
	...
	log.InfoContext(ctx, "begin tran")
	tx, err := db.Beginx()
	if err != nil {
	...
	defer func() {
		if err := tx.Rollback(); err != nil {
			...
			log.ErrorContext(ctx, "unable to rollback tran", "trace_id", traceID, "ERROR", err)
		}
		log.InfoContext(ctx, "rollback tran", "trace_id", traceID)
	}()
	...
	log.InfoContext(ctx, "commit tran", "trace_id", traceID)
...
func NamedExecContext(ctx context.Context, log *slog.Logger, db sqlx.ExtContext, query string, data any) error {
	...
	// }
	log.InfoContext(ctx, "database.NamedExecContext", "trace_id", web.GetTraceID(ctx), "query", q)
...
func namedQuerySlice[T any](ctx context.Context, log *slog.Logger, db sqlx.ExtContext, query string, data any, dest *[]T, withIn bool) error {
	...
	log.InfoContext(ctx, "database.NamedQuerySlice", "trace_id", web.GetTraceID(ctx), "query", q)
	var rows *sqlx.Rows
	var err error
